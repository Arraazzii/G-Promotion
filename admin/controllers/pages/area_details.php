<?php
    if (!isset($_POST['kode_area'])) {
        echo "<script>window.history.back();</script>";
    }else{
    $kode = $_POST['kode_area'];
    $q = mysql_fetch_array(mysql_query("SELECT * FROM m_area JOIN alamat ON m_area.kode_alamat = alamat.kode_alamat WHERE m_area.id = '$kode' ")) or die(mysql_error());
